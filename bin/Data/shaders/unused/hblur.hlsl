...
RWTexture2D<float4> gBlurred;					// Destination image
...
void CSMain(int3 dispatch_thread_id : SV_DispatchThreadID, int3 group_thread_id : SV_GroupThreadID){
	...
	if (group_thread_id.x < kBlurRadius) {
		...
		samples[group_thread_id.x] = gSource[int2(max(dispatch_thread_id.x - kBlurRadius, 0),
												  dispatch_thread_id.y)];

	}
	else if (group_thread_id.x >= N - kBlurRadius) {

		samples[group_thread_id.x + 2 * kBlurRadius] = gSource[int2(min(dispatch_thread_id.x + kBlurRadius, gSource.Length.x - 1),
																	dispatch_thread_id.y)];

	}

	samples[group_thread_id.x + kBlurRadius] = gSource[min(dispatch_thread_id.xy, gSource.Length.xy - 1)];
	...
	// Write the output
	gBlurred[dispatch_thread_id.xy] = color;
